  ...
  return (
    <Shell>
      <div className="space-y-5 animate-in">
        ...
        <Card><div className="section-header mb-3 flex items-center gap-2"><Truck className="w-3.5 h-3.5"/> Scheduled Installs</div>{((data?.installs_this_week as unknown[])?.length||0)===0?<div className="text-center py-8 text-muted text-sm">No installs scheduled.</div>:<div className="space-y-1">{(data?.installs_this_week as Array<Record<string,unknown>>)?.map((inst:Record<string,unknown>)=>(<div key={inst.id as string} className="data-row rounded px-2"><div><span className="text-sm font-medium">{inst.project_name as string}</span><div className="text-[11px] text-muted">{inst.scheduled_date as string} — {inst.lead_installer as string||"Unassigned"}</div></div><StatusBadge status={inst.status as string}/></div>))}</div>}</Card>
      </div>
